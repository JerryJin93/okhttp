...
class RealCall(
  ...
  internal fun getResponseWithInterceptorChain(): Response {
    ...
    // network interceptors在ConnectInterceptor与CallServerInterceptor之间

    // chain -> RealInterceptorChain
    // chain proceeds with request, interceptor intercepts chain
    // proceed -> generates next chain, gets current interceptor, and intercept next chain
    // the next chain is generated by forking the current chain, the only difference is the index has been plussed one.
  ...
  fun enterNetworkInterceptorExchange(
    ...
    // 总而言之，此方法用于初始化RoutePlanner以及ExchangeFinder
  }
  ...
  inner class AsyncCall(
    ...
    override fun run() {
      threadName("OkHttp ${redactedUrl()}") {
        ...
        timeout.enter()
        try {
          // compose the interceptor chain and start to request.
